...
const TicketsEdit=(props)=>{
  ...
  return <div><div className="TicketsEdit">
      {props.currentQuestion&&props.ticketlist&&props.ticketlist.map(tl=>{
        return !tl.deleted&&<div>
              <input onChange={(e)=>props.addQuestionToTicket(e.target.checked,tl.id)} type="checkbox" checked={tl.questions.some(t=>t==props.currentQuestion.id)}/>
              <TicketsEditName newt={tl.added} value={tl.ticketname} edit={(e)=>props.editTicket(tl.id,e.currentTarget.value)}/>
              <button onClick={()=>{props.deleteTicket(tl.id)}}>&#215;</button>
            </div>})}
          </div>
