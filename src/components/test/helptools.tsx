...
import {h_FiftyFifty_AC,h_Wrong_AC,h_PhoneCall_AC,h_Replace_AC} from "../../redux/reducerTests2"
...
const HelpTools:FC<THelpTools>=({h_FiftyFifty,h_Wrong,h_PhoneCall,h_Replace,h_Money})=>{
...
const bmoney=()=>{
 ...
 return <div className="HelpTools">
  <div className={bMoney?"btnMoney disabled":"btnMoney"} onClick={()=>{bmoney()}} data-title="Забрать деньги" title="Забрать деньги"></div>
  <div className={b5050?"btn5050 disabled":"btn5050"} onClick={()=>{b1()}} data-title="Убрать два неверных ответа"></div>
  <div className={bReplace?"btnreplace disabled":"btnreplace"} onClick={()=>{b4()}} data-title="Замена вопроса"></div>
  <div className={bPhoneCall?"btnphonecall disabled":"btnphonecall"} onClick={()=>{b3()}} data-title="Звонок другу"></div>
  <div className={bWrong?"btnwrong disabled":"btnwrong"} onClick={()=>{b2()}} data-title="Право на ошибку"></div>
 </div>
}
