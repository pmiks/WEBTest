...
const TestEdit:FC<TTestEdit>=({tp,setTP,saveTestCoverImg,deleteTestCoverImg})=>{
  ...
  let testTypeCont=testtype.map((tt)=>{return <option value={tt.id} selected={tt.id==testTypeId?true:false}>{tt.name}</option>})
  let saveTCI=(e:any,id:number)=>{alert(id); if(e.target.files.length) saveTestCoverImg(id,e.target.files[0]);  }
  return tp?<div className="testedit">
         <LoadImage zoomer="true" id={"TEST"+tp.id} width="100px" height="100px" img={tp.coverimg} onLoad={(e:any)=>{saveTCI(e,tp.id)}} onDel={()=>{deleteTestCoverImg(tp.id)}}/>
        <div><select id="typeTest" onChange={selectTypeTest}> {testTypeCont} </select></div>
      ...
      <div><label>Название теста</label>
           ...
           <div><label>Описание, хэштеги:</label>
                <input value={tp.hashtag} type="input" onChange={(e)=>{setTP({...tp,hashtag:e.currentTarget.value})}}/></div>
      <div><label>Количество вариантов ответов по умолчанию</label>
           <input style={{"width":"2em"}} type="input" value={tp.defaultAnswerCol} onChange={(e)=>{setTP({...tp,defaultAnswerCol:toNumLimMinMax(e.currentTarget.value,0,10)})}}/></div>
      ...
      <div><label>Ответ по  умолчанию</label>
           ...
           <div><label>Автоматически добавлять неверные варианты ответа:</label>
                <input style={{"width":"2em"}} value={tp.addalter} onChange={(e)=>{setTP({...tp,addalter:toNumLimMinMax(e.currentTarget.value,0,10)})}} type="input"/></div>
      <div><label>Время для ответа на вопрос</label>
          <input style={{"width":"4em"}} value={tp.questiontime} onChange={(e)=>{setTP({...tp,questiontime:toNumLimMinMax(e.currentTarget.value,0,90000000)})}} type="input"/></div>
      ...
      {testTypeId in [0]&&<div><label>Количество допустимых ошибок</label>
           ...
           <div><label>Время отображения окна результата вопроса</label>
                <input value={tp.timeQuestResult} onChange={(e)=>{setTP({...tp,timeQuestResult:toNumLimMinMax(e.currentTarget.value,0,90000000)})}} type="input"/></div>
      <div><label>Отбирать случайным образом</label>
           <input style={{"width":"5em"}} value={tp.limit_quest} onChange={(e)=>{setTP({...tp,limit_quest:toNumLimMinMax(e.currentTarget.value,0,90000000)})}} type="input"/><label>вопросов</label></div>
